# Future evolution of glaciers in the European Alps

OGGM edu graphic: illustrates the evolution of the volume of the European alpine glaciers by using the data of [Rounce, D.R., R. Hock, and F. Maussion. 2022. Global PyGEM-OGGM Glacier Projections with RCP and SSP Scenarios, Version 1](https://doi.org/10.5067/P8BN9VO9N5C7) or of [Zekollari et al., 2019: Modelling the future evolution of glaciers in the European Alps under the EURO-CORDEX RCM ensemble](https://tc.copernicus.org/articles/13/1125/2019/)

(for more information, visit [edu.oggm.org](https://edu.oggm.org))

Author:

- [Lilian Schuster](https://github.com/lilianschuster) (University of Innsbruck)
- [Zora Schirmeister](https://github.com/zschirmeister) (University of Innsbruck)
- [Fabien Maussion](https://github.com/fmaussion) (University of Innsbruck)

License: BSD3

[![badge](https://img.shields.io/badge/launch-OGGM%20Edu%20app-cc514e.svg?logo=data:image/png;base64,)](https://edu.oggm.org/en/latest/alps_future-app.html)


you have issues to run the notebooks, then do:
- `conda env create --name oggm_edu_alps_future -f environment.yml`
- `conda activate oggm_edu_alps_future`
- -> and then run the notebooks in that newly created environment

## Possible future improvements:
- implementation of uncertainties into the graphic
- possibility to choose between absolute and percentage change of volume
- adding a map in which the user can select glaciers
- implement tick-boxes for choosing the displayed RCP-scenarios
- two dropdown menus to give the opportunity that every region can be compared to every other region/country

If you are interested to contribute you can send us the edited [file](alps_future.ipynb) per mail or with a pull-request. Thanks!
